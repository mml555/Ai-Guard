...
try {
  ...
  if (err instanceof PolicyBlockedError) {
    const auditId = err.body.error?.auditRequestId;
    return res.status(402).json({
      error: "ai_policy_blocked",
      reasonCode: err.body.error?.reasonCode,
...
On policy blocks, use `error.auditRequestId` (same format). Also available as
the `x-ai-guard-request-id` response header.
