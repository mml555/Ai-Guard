...
| `feature_budget_exceeded` | Feature monthly cap | Raise feature budget |
| `global_budget_exceeded` | Project-wide stop | Ops: raise global cap |
...
Success body: `requestId`. Error body: `auditRequestId` (audit) + `requestId` (HTTP trace).
