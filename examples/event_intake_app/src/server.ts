...
${flyerText}`;
  ...
  // Correlate host app entity ↔ Ai-Guard audit row for debugging.
    ...
    if (err instanceof PolicyBlockedError) {
      const body = err.body as { error?: { auditRequestId?: string; reasonCode?: string } };
      const auditId = body.error?.auditRequestId;
      console.warn(
        `jewgo_event_draft=${draftId} policy_blocked reason=${body.error?.reasonCode} ai_guard_request_id=${auditId ?? "n/a"}`,
      );
