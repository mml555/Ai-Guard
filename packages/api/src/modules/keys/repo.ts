...
export async function listApiKeys(
  pool: Pool,
  opts: { includeRevoked?: boolean; projectId?: string } = {},
...
export async function getApiKeyById(
  ...
  const { rows } = await pool.query<ApiKeyDbRow>(
    `SELECT ${SELECT_FIELDS} FROM api_keys WHERE id = $1`,
    [id],
  );
...
export async function revokeApiKey(pool: Pool, id: string): Promise<boolean> {
  ...
  // Distinguish "already revoked" (still a success) from "unknown id".
  const exists = await pool.query("SELECT 1 FROM api_keys WHERE id = $1", [id]);
