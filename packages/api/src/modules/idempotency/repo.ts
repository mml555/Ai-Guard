...
const CLAIM_SQL = `
  INSERT INTO idempotency_keys (key, user_id, request_hash, status, claimed_at)
...
export async function claimKey(
  pool: Pool,
  params: { key: string; userId: string; requestHash: string },
  attempt = 0,
): Promise<ClaimResult> {
  ...
  const { rows } = await pool.query(SELECT_SQL, [params.userId, params.key]);
  const row = rows[0] as
    | {
        request_hash: string;
        status: "processing" | "completed";
